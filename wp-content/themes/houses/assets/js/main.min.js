!(function (t, e) {
  ...
  class tt extends m {
    ...
    createSlideEl(t) {
      ...
      o.length &&
        (r =
          i[o.reduce((t, e) => (Math.abs(e - a) < Math.abs(t - a) ? e : t))]);
      const l =
  ...
  class te extends $ {
    ...
    createEl(t) {
      ...
      if ("slideshow" === t && (!i.plugins.Autoplay || o < 2)) return null;
      if (void 0 !== Zt[t]) {
        const e = Zt[t];
        ((n = document.createElement("button")),
          n.setAttribute(
...
(document.addEventListener("DOMContentLoaded", () => {
  ...
  accordionHeaders.forEach((header) => {
    header.addEventListener("click", (e) => {
      ...
      const targetId = header.dataset.accordion,
        ...
        isAlreadyActive = targetContent.classList.contains("active");
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function createDatepicker(wrapper) {
      const dateInput = wrapper.querySelector("._input"),
        calendarIcon = wrapper.querySelector("._calendar");
      if (!dateInput || !calendarIcon) return;
      "static" === getComputedStyle(wrapper).position &&
        (wrapper.style.position = "relative");
      const calendarBlock = document.createElement("div");
      ((calendarBlock.className =
        "custom-datepicker custom-datepicker--hidden"),
      ...
      function renderCalendar() {
        ...
        let startWeekday = new Date(year, month, 1).getDay(),
          startOffset = 0 === startWeekday ? 6 : startWeekday - 1;
        const daysInMonth = new Date(year, month + 1, 0).getDate(),
          daysContainer = calendarBlock.querySelector(
            ".custom-datepicker__days",
          );
        ...
        for (let i = 0; i < 42; i++)
          if (i < startOffset || i >= startOffset + daysInMonth) {
            ...
            ((emptyDiv.className =
              "custom-datepicker__day custom-datepicker__day--empty"),
        ...
        const monthSpan = calendarBlock.querySelector(
            ".custom-datepicker__month",
          ),
          yearSpan = calendarBlock.querySelector(".custom-datepicker__year");
        (monthSpan &&
      ...
      function selectDate(date) {
        (console.log("selectDate вызвана, дата:", date),
          (selectedDate = date),
          (dateInput.value = (function (date) {
            return `${String(date.getDate()).padStart(2, "0")}.${String(date.getMonth() + 1).padStart(2, "0")}.${date.getFullYear()}`;
          })(date)),
          console.log("До добавления hidden, классы:", calendarBlock.classList),
          calendarBlock.classList.add("hidden"),
          console.log(
            "После добавления hidden, классы:",
            calendarBlock.classList,
          ));
      }
      const prevBtn = calendarBlock.querySelector(
          ".custom-datepicker__nav--prev",
        ),
        nextBtn = calendarBlock.querySelector(".custom-datepicker__nav--next");
      (prevBtn &&
        ...
        calendarIcon.addEventListener("click", function (e) {
          e.stopPropagation();
          const isHidden = calendarBlock.classList.contains(
            "custom-datepicker--hidden",
          );
          (!(function (exceptPicker = null) {
            datepickers.forEach((picker) => {
              picker !== exceptPicker &&
                picker.calendarBlock.classList.add("custom-datepicker--hidden");
            });
          })(datepickers.find((p) => p.calendarBlock === calendarBlock)),
            isHidden
    ...
    (document.addEventListener("click", function (e) {
      datepickers.forEach((picker) => {
        picker.wrapper.contains(e.target) ||
          picker.calendarBlock.classList.add("custom-datepicker--hidden");
      });
    }),
  ...
  document.querySelector("#to-exit") &&
...
function getZoomByWidth(width) {
  return width <= 465 ? 12.45 : 13.45;
}
function initMap() {
  if ("undefined" == typeof ymaps) return;
  const mapContainer = document.getElementById("map");
  if (!mapContainer) return;
  const zoom = getZoomByWidth(mapContainer.clientWidth);
    ...
    window.addEventListener("resize", function () {
      (resizeTimeout && clearTimeout(resizeTimeout),
        (resizeTimeout = setTimeout(function () {
          const newZoom = getZoomByWidth(mapContainer.clientWidth);
          myMap.getZoom() !== newZoom && myMap.setZoom(newZoom);
        }, 200)));
    }));
}
(window.addEventListener("scroll", function () {
  window.scrollY > 10 ? nav.classList.add("fix") : nav.classList.remove("fix");
}),
  document.querySelectorAll(".form__tz")?.forEach(function (form) {
    const fileInput = form.querySelector(".file"),
      fileNameElement = form.querySelector(".file-name");
    fileInput &&
      fileNameElement &&
    ...
    (entries) => {
      entries.forEach((entry) => {
        entry.isIntersecting &&
          (ymaps.ready(initMap), mapObserver.unobserve(mapElement));
      });
    },
...
if (phoneInputs && phoneInputs.length) {
  const getInputsNumbersValue = function (input) {
    ...
    onPhoneInput = function (e) {
      ...
      if (input.value.length == selectionStart) {
        if (["7", "8", "9"].indexOf(inputNumbersValue[0]) > -1) {
          "9" == inputNumbersValue[0] &&
            (inputNumbersValue = "7" + inputNumbersValue);
          ((formattedInputValue =
            ("8" == inputNumbersValue[0] ? "8" : "+7") + " "),
            inputNumbersValue.length > 1 &&
              (formattedInputValue +=
                "(" +
                ("8" == inputNumbersValue[1]
                  ? (inputNumbersValue[1] = "")
                  : inputNumbersValue[1]) +
                inputNumbersValue.substring(2, 4)),
            inputNumbersValue.length >= 5 &&
              (formattedInputValue += ") " + inputNumbersValue.substring(4, 7)),
            inputNumbersValue.length >= 8 &&
              (formattedInputValue += "-" + inputNumbersValue.substring(7, 9)),
            inputNumbersValue.length >= 10 &&
              (formattedInputValue +=
    ...
    onPhoneKeyDown = function (e) {
      ...
      8 == e.keyCode &&
        1 === getInputsNumbersValue(input).length &&
        (input.value = "");
    },
    onPhonePaste = function (e) {
      ...
      if (pasted) {
        ...
        /\D/g.test(pastedText) || (input.value = inputNumbersValue);
      }
    },
...
(document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll("._select").forEach((input) => {
    ...
    (dropdown.addEventListener("mousedown", (e) => {
      ...
      input.addEventListener("click", (e) => {
        (e.stopPropagation(),
          justSelected ||
            (dropdown.classList.contains("custom-select--hidden")
              ? justSelected ||
      ...
      document.addEventListener("click", (e) => {
        wrapper.contains(e.target) || (hideDropdown(), (justSelected = !1));
      }),
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    (new Swiper(".houses__swiper.swiper", {
      slidesPerView: 3,
      spaceBetween: 21,
      speed: 1e3,
      navigation: {
      ...
      breakpoints: { 991: { spaceBetween: 21 }, 320: { spaceBetween: 10 } },
    }),
      new Swiper(
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const buttons = document.querySelectorAll(".tabs__btn"),
      panes = document.querySelectorAll(".tabs__pane");
    function switchTab(tabId) {
      panes.forEach((pane) => pane.classList.remove("active"));
      const activePane = document.querySelector(
        `.tabs__pane[data-tab="${tabId}"]`,
      );
      (activePane && activePane.classList.add("active"),
        buttons.forEach((btn) => btn.classList.remove("active")));
      const activeButton = document.querySelector(
        `.tabs__btn[data-tab="${tabId}"]`,
      );
    ...
    (buttons.forEach((btn) => {
      ...
      buttons.length && switchTab(buttons[0].getAttribute("data-tab")));
  }));
